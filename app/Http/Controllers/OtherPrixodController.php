...
class OtherPrixodController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $prixodi = Prixod::where('prixod_type', '3')->with('photos')->orderBy('created_at', 'desc')->paginate(15);;

        return view('otherprixodi.home', [
            'url'           => '/prixodi/otherprixodi/printprixodi',
            'fa'            => 'fa-money',
            'title'         => 'Karz ПРИХОД',
            'prixodi'       => $prixodi,
        ]);
    ...
    // Add karz prixodi 
    public function addotherprixodi(Request $request) {
    ...
    public function printprixodi(Request $request) {
        if ($request->isMethod('get')) {
            ...
            $prixodi = Prixod::where('prixod_type', '3')->whereBetween('data', [$period_starts_at, $period_ends_at])->orderBy('created_at', 'desc')->get();

            return view('otherprixodi.pdfjquery', [
                'title'     => 'ПРИХОД',
                'urlprint'  => '/prixodi/otherprixodi/printprixodi',
                'prixodi'   => $prixodi
            ]);
        }
    }
