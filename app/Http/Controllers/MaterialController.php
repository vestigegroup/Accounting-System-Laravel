...
class MaterialController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($project) {
            $rasxodi = Rasxod::where(['rasxod_type' => '2', 'project_id' => $project_id])->with('photos')->where('project_id', $project_id)->orderBy('created_at', 'desc')->paginate(15);;

            return view('material.home', [
                'url'           => $project_id.'/rasxodi/materials/printmaterial',
    ...
    public function addmaterial(Request $request) {
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                $rasxod->rasxod_type    = 2; 
                $rasxod->name           = $request->name;
                $rasxod->chenak         = $request->chenak;

                $chenak = $request->mikdor;
                $narx   = $request->narx;
                $summa  = $chenak * $narx;
 
                $rasxod->chenak         = $chenak;
                $rasxod->narx           = $narx;
                $rasxod->summa          = $summa;
                ...
                return redirect()->back()->with('message', 'Добавлено успешно!!');
    ...
    // Print Material
    public function printmaterial(Request $request) {
        if ($request->isMethod('get')) {
            ...
            if($project) {
                ...
                $zarplatarasxods = Rasxod::where(['rasxod_type' => '2', 'project_id' => $project_id])->where('project_id', $project_id)->whereBetween('data', [$period_starts_at, $period_ends_at])->orderBy('created_at', 'desc')->get();

                return view('pdfjquery', [
                    'title'             => 'МАТЕРИАЛЫ Расходи',
                    'urlprint'          => $project_id.'/rasxodi/materials/printmaterial',
