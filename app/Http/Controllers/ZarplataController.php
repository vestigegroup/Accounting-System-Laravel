...
class ZarplataController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($project) {
            $rasxodi = Rasxod::where(['rasxod_type' => '1', 'project_id' => $project_id])->with('photos')->where('project_id', $project_id)->orderBy('created_at', 'desc')->paginate(15);;

            return view('zarplata.home', [
                'url'           => $project_id.'/rasxodi/zarplata/printzarplata',
                'fa'            => 'fa-user',
                'title'         => 'Зарплата',
                'rasxodi'       => $rasxodi,
    ...
    // Add zarplata 
    public function addzarplata(Request $request) {
    ...
    public function printzarplata(Request $request) {
        if ($request->isMethod('get')) {
            ...
            if ($project) {
                ...
                $zarplatarasxods = Rasxod::where('rasxod_type', '1')->where('project_id', $project_id)->whereBetween('data', [$period_starts_at, $period_ends_at])->orderBy('created_at', 'desc')->get();

                return view('pdfjquery', [
                    'title'             => 'Зарплата Расходи',
                    'urlprint'          => $project_id.'/rasxodi/zarplata/printzarplata',
